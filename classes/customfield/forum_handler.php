...
/**
 * Forum handler for custom fields
 *
 * @package   mod_forum
...
namespace mod_forum\customfield;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class forum_handler extends \core_customfield\handler {
    ...
    /** @var int Field is displayed in the forum listing, visible to everybody */
    const VISIBLETOALL = 2;
    /** @var int Field is displayed in the forum listing but only for teachers */
    const VISIBLETOTEACHERS = 1;
    /** @var int Field is not displayed in the forum listing */
    const NOTVISIBLE = 0;
     ...
     *
     * @param int $itemid
     * @return \mod_forum\customfield\forum_handler
     */
    ...
    /**
     * The current user can edit custom fields on the given forum.
     *
    ...
    /**
     * The current user can view custom fields on the given forum.
     *
     * @param field_controller $field
     * @param int $instanceid id of the course to test edit permission
     * @return bool true if the current can edit custom fields, false otherwise
    ...
    /**
     * Sets parent context for the forum
     *
    ...
    public function get_configuration_url() : \moodle_url {
        return new \moodle_url('/mod/forum/customfield.php');
    }
    ...
    public function config_form_definition(\MoodleQuickForm $mform) {
        $mform->addElement('header', 'forum_handler_header', get_string('customfieldsettings', 'mod_forum'));
        $mform->setExpanded('forum_handler_header', true);

        // If field is locked.
        $mform->addElement('selectyesno', 'configdata[locked]', get_string('customfield_islocked', 'mod_forum'));
        $mform->addHelpButton('configdata[locked]', 'customfield_islocked', 'mod_forum');

        // Field data visibility.
        $visibilityoptions = [self::VISIBLETOALL => get_string('customfield_visibletoall', 'mod_forum'),
            self::VISIBLETOTEACHERS => get_string('customfield_visibletoteachers', 'mod_forum'),
            self::NOTVISIBLE => get_string('customfield_notvisible', 'mod_forum')];
        $mform->addElement('select', 'configdata[visibility]', get_string('customfield_visibility', 'mod_forum'),
            $visibilityoptions);
        $mform->addHelpButton('configdata[visibility]', 'customfield_visibility', 'mod_forum');
    }
